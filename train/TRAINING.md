...
A detailed guide to how we train FlightMind from random weights to a functional language model. Every decision is explained here, because the document is meant to teach, not just document.
...
The default Adam uses β₂ = 0.999, which averages over the last ~1000 steps. For LLM training, this is too slow, since the loss landscape changes faster than the variance estimate can track. β₂ = 0.95 (averaging over ~20 steps) is more responsive and is now standard for LLM pretraining.
...
### Phase 1: Linear Warmup (Steps 0 to 500)
...
Empirically, 500-2000 warmup steps works well. Too few and you get instability. Too many and you waste compute at low LR.

### Phase 2: Cosine Decay (Steps 500 to 50,000)
...
Adam's m and v estimates take hundreds of steps to warm up. If we resumed training with fresh optimizer state, the first few hundred steps would effectively be "re-warming up," wasting compute and risking loss spikes. Saving and restoring optimizer state lets us resume as if training never stopped.

### Checkpoint Strategy

- **`best.pt`**: Saved whenever validation loss improves. This is the checkpoint we'll use for deployment.
- **`step_N.pt`**: Saved every 1,000 steps. Insurance against crashes, so we never lose more than 1,000 steps of work.
...
2. **Model selection**: The checkpoint with the lowest validation loss is our best model, since it generalizes best to unseen text.
...
With only 99M tokens and a 566M parameter model, we expect to see some overfitting after 1-2 epochs. This is normal and expected, because we're deliberately training a larger model than our data technically supports to maximize the model's capacity for aviation knowledge.
...
Critically, we only compute loss on the **assistant tokens**. The system prompt and user message are context, and training on them would teach the model to generate user messages rather than respond to them. This is implemented via a binary loss mask.

### Merged Deployment

After fine-tuning, LoRA weights are **merged** into the base model: `W_new = W + A @ B`. The resulting model has identical architecture to the original, with no LoRA overhead at inference time. This merged checkpoint is what would be deployed.
